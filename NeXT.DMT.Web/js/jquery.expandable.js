    ...
    $.fn.extend({
        expandable: function (givenOptions) {
            ...
            return this.filter('textarea').each(function () {
                ...
                // setup events
                $this
                .bind('keypress', function (event) { if ((event.keyCode == '13') || (event.keyCode == '8')) { check(); } })
                .bind('focus blur', function (event) {
                    //if (event.type == 'blur') clearInterval(interval);
                    if (event.type == 'focus') check(); //interval = setInterval(check, options.interval);
                    if (event.type == 'keypress') check();
                });
